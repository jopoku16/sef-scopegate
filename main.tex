...
\begin{proposition}[Conflict is not identified by confidence]
...
C=1-\frac{|n|}{m}.
\]
Therefore a model score, margin, or confidence value that determines $n$ does not determine conflict unless evidence mass is itself determined by that score.
\end{proposition}
...
This proposition resolves a basic identification question. Conflict and confidence may be computed from the same features and fitted parameters, but they compress those inputs differently. Confidence records the net score. Conflict also depends on how much evidence cancels before that net score is reached. The proposition does not prove that conflict predicts error. It proves only that conflict can contain information that is absent from confidence. Whether that information is useful must be tested on held-out outcomes.
...
Suppose the decision is based on the sign of net evidence:
\[
...
\]
Other bounded risk scores may be used, including prediction error weighted by evidence unreliability.
...
\begin{theorem}[Distribution-free screening guarantee]
...
\resizebox{\textwidth}{!}{\input{results/sef_conformal_screen_table.tex}}
\end{table}
...
The result is not perfect, and it should not be oversold. The screen works clearly on three of the four standard tasks. On Wine, the flagged group is small and not more error-prone on average. This is still useful: it shows that the conformal screen gives a calibrated evidence flag, not a guarantee that every flagged group will have higher prediction error in every small data set.
...
\resizebox{\textwidth}{!}{\input{results/sef_standard_benchmarks_table.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{results/sef_selective_baseline_table.tex}}
\end{table}
...
\input{results/sef_covtype_table.tex}
\end{table}
...
\resizebox{\textwidth}{!}{\input{results/sef_multiclass_benchmark_table.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{results/sef_model_agnostic_robustness_table.tex}}
\end{table}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.86\textwidth]{figures/figure10_evidence_reliability_frontier.png}
\caption{Evidence Reliability Frontier for SEF-Audit. The frontier shows accepted-case error as a function of the fraction of cases sent to review. The curves summarize standard data sets, the large Covertype benchmark, and the repeated black-box robustness experiment.}
\label{fig:evidence-frontier}
\end{figure}

Figure~\ref{fig:evidence-frontier} summarizes the operational role of SEF-Audit. The same review-budget idea applies across small standard data sets, the large Covertype benchmark, and repeated black-box models. The curve is not meant to replace accuracy or calibration metrics. It answers a different question: how much review is needed to make the automatically accepted cases safer?
...
\label{tab:ablation}
\input{results/sef_ablation_table.tex}
\end{table}
...
\input{results/sef_confidence_masking_table.tex}
\end{table}
...
\resizebox{\textwidth}{!}{\input{results/sef_healthcare_beyond_confidence_table.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{results/sef_entropy_comparison_table.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{results/sef_finance_credit_benchmark_table.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{results/sef_scope_diagnostic_table.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{results/sef_external_replication_table.tex}}
\end{table}
...
The main message is practical. SEF reveals whether evidence is one-sided, opposed, or unstable, and the identification result shows that this structure is not recoverable from confidence alone. But evidence structure is not the same as error risk. A held-out calibration sample must decide whether conflict has a positive, negative, or inconclusive relationship with error in the target population. SEF provides the description; ScopeGate controls the deployment claim.
